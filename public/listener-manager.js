...
function renderDetail(data) {
  ...
  el.detailContent.innerHTML = `
    ...
    <div class="detail-metrics">${textMetric("総合ランク",contributionDetail(item,"lifetime"))}${textMetric("直近30日",contributionDetail(item,"recent"))}${metric("ランキング対象コイン",item.contributionCoins)}${metric("30日対象コイン",item.recentContributionCoins)}${metric("来訪",totals.visits)}${metric("コメント",totals.comments)}${metric("ギフト個数",totals.gifts)}${metric("全ギフトコイン",totals.coins)}${metric("スタンプ",stampTotal)}${metric("印刷",receiptTotal)}</div>
