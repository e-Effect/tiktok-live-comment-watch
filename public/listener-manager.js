...
async function setInlineGoodImpression(input) {
  ...
  const item = state.items.find(candidate => candidate.userId === userId);
  if (!item) return;
  const previous = Boolean(item.goodImpression);
  input.disabled = true;
  try {
    const response = await api(`/api/listeners/${encodeURIComponent(userId)}`, {
      method:"PATCH", headers:{"Content-Type":"application/json"},
      body:JSON.stringify({goodImpression:Boolean(input.checked)})
    });
    if (!response.ok) throw new Error("いい印象の設定を保存できませんでした");
    const updated = await response.json();
    item.goodImpression = Boolean(updated.goodImpression);
    renderListenerTable();
  } catch (error) {
    input.checked = previous;
    showConnectionError(error);
