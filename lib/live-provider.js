...
class TikToolsConnection extends EventEmitter {
  ...
  forwardEvents() {
    ...
    this.client.on("superFan", (event) => this.emit("superFan", event));
    this.client.on("superFanJoin", (event) => this.emit("superFanJoin", event));
    this.client.on("fansEvent", (event) => this.emit("fansEvent", event));
