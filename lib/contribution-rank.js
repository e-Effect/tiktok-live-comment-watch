...
export function contributionTier(position, total) {
  ...
  if (rank <= Math.max(1, Math.ceil(population * 0.01))) return "S";
  if (rank <= Math.max(1, Math.ceil(population * 0.05))) return "A";
  if (rank <= Math.max(1, Math.ceil(population * 0.15))) return "B";
  if (rank <= Math.max(1, Math.ceil(population * 0.40))) return "C";
  return "D";
}
