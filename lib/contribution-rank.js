...
export function buildContributionRankings(rows = []) {
  const prepared = rows.map((row) => ({
    ...
    coinsPerVisit: coinsPerVisit(row.coins, row.visits),
    ...
    recentCoinsPerVisit: coinsPerVisit(row.recent_coins || row.recentCoins, row.recent_visits || row.recentVisits),
  ...
  applyScores(recentEligible, {
    ...
    coins: "recentCoins",
    coinsPerVisit: "recentCoinsPerVisit",
  ...
  rankRows(lifetimeEligible, {
    ...
    tieBreakers: ["coins", "coinsPerVisit", "comments", "visits", "likes", "lastSeenAt"]
  });
  rankRows(recentEligible, {
    ...
    tieBreakers: ["recentCoins", "recentCoinsPerVisit", "recentComments", "recentVisits", "recentLikes", "recentLastSeenAt"]
  });
  ...
  for (const row of prepared) {
    if (!lifetimeSet.has(row.userId)) {
      row.contributionScore = silentVisitPenalty(row.visits);
      row.contributionPosition = null;
      row.contributionTotal = lifetimeEligible.length;
      row.contributionRank = hasLifetimePresence(row) ? "D" : "集計不足";
    }
    if (!recentSet.has(row.userId)) {
      row.recentContributionScore = silentVisitPenalty(row.recentVisits);
      row.recentContributionPosition = null;
      row.recentContributionTotal = recentEligible.length;
      row.recentContributionRank = hasRecentPresence(row) ? "D" : "活動なし";
    }
  }
  ...
  const recentOrder = [...prepared].sort((left, right) =>
    nullablePosition(left.recentContributionPosition) - nullablePosition(right.recentContributionPosition)
      || right.recentContributionScore - left.recentContributionScore
      || nullablePosition(left.contributionPosition) - nullablePosition(right.contributionPosition)
      || right.lastSeenAt - left.lastSeenAt
      || left.userId.localeCompare(right.userId)
  ).map((row) => row.userId);
...
export function contributionTier(position, total) {
  ...
  if (rank <= Math.max(1, Math.ceil(population * 0.005))) return "S";
  if (rank <= Math.max(1, Math.ceil(population * 0.02))) return "A";
  if (rank <= Math.max(1, Math.ceil(population * 0.06))) return "B";
  if (rank <= Math.max(1, Math.ceil(population * 0.10))) return "C";
  return "D";
}
...
function applyScores(rows, fields) {
  ...
  const distributions = {
    ...
    coinsPerVisit: cumulativeDistribution(rows, fields.coinsPerVisit),
  ...
  for (const row of rows) {
    row[fields.score] = roundScore(
      ...
      + distributions.coinsPerVisit.get(row.userId) * LIFETIME_WEIGHTS.coinsPerVisit
...
function cumulativeDistribution(rows, field) {
  const sorted = rows.filter((row) => row[field] > 0).sort((left, right) => left[field] - right[field]);
  const result = new Map(rows.map((row) => [row.userId, 0]));
...
function publicRanking(row) {
  return {
    ...
    contributionCoins: row.coins,
    contributionCoinsPerVisit: roundScore(row.coinsPerVisit),
    ...
    recentContributionCoins: row.recentCoins,
    recentContributionCoinsPerVisit: roundScore(row.recentCoinsPerVisit)
  };
}

function isLifetimeEligible(row) {
  return row.coins > 0 || row.comments > 0;
}

function isRecentEligible(row) {
  return row.recentCoins > 0 || row.recentComments > 0;
}

function hasLifetimePresence(row) {
  return row.visits >= 2 || row.likes > 0 || row.lastSeenAt > 0;
}

function hasRecentPresence(row) {
  return row.recentVisits > 0 || row.recentLikes > 0 || row.recentLastSeenAt > 0;
}

function silentVisitPenalty(visits) {
  const repeatedVisits = Math.max(0, Math.floor(Number(visits || 0)) - 2);
  return -Math.min(30, repeatedVisits * 2);
}
...
function coinsPerVisit(coins, visits) {
  const totalCoins = nonNegativeNumber(coins);
  const visitCount = Math.max(1, nonNegativeNumber(visits));
  return totalCoins / visitCount;
}
