...
export class EventStore {
    ...
    if (!this.contributionRankRestorePromises.has(cacheKey)) {
      const restore = (async () => {
        const result = await this.pool.query(
          ...
          ["contribution-ranks:v1:" + cacheKey]
        );
  ...
  async refreshListenerContributionRankings(username, cacheKey = String(username || "").trim().toLowerCase() || "*") {
    ...
    const refresh = (async () => {
      ...
      WHERE LOWER(l.user_id) <> 'unknown'
        AND ($1 = '' OR t.user_id IS NOT NULL)
      `, [username]);
      const rankings = {
      ...
      `,["contribution-ranks:v1:" + cacheKey,JSON.stringify({entries:[...rankings.byUserId],lifetimeOrder:rankings.lifetimeOrder,recentOrder:rankings.recentOrder,generatedAt:rankings.generatedAt})]);
