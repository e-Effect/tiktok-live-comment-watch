...
export class EventStore {
  ...
  async listeners({ username = "", search = "", sort = "last_seen", direction = "desc", classification = "all", blocked = "all", attention = "all", giftExperience = "all", commentExperience = "all", limit = 100, offset = 0, fresh = false } = {}) {
      ...
      const fastResult = await this.pool.query(`
        WITH page AS MATERIALIZED (
          SELECT l.user_id, l.latest_unique_id, l.latest_nickname, l.avatar_url,
            ...
            l.first_seen_at, l.last_seen_at, l.is_super_fan, l.needs_attention, l.good_impression, l.gift_excluded, l.is_super_lurker, l.is_blocked, l.notes, l.tags,
    ...
    const result = await this.pool.query(`
      ...
      SELECT p.*, l.latest_unique_id, l.latest_nickname, l.avatar_url,
        (l.avatar_data IS NOT NULL) AS avatar_cached,
        l.first_seen_at, l.last_seen_at, l.is_super_fan, l.needs_attention, l.good_impression, l.gift_excluded,
  ...
  async listenerExportRows({ username = "", search = "" } = {}) {
    ...
    const result = await this.pool.query(`
      ...
      SELECT l.user_id, l.latest_unique_id, l.latest_nickname,
        l.is_super_fan, l.needs_attention, l.good_impression, l.gift_excluded, l.is_super_lurker, l.is_blocked, l.notes, l.tags,
        l.host_follow_status, l.host_follow_status_updated_at,
  ...
  async listenerDetail(userId, { username = "" } = {}) {
    ...
    const listener = await this.pool.query(`
      SELECT user_id, latest_unique_id, latest_nickname, avatar_url,
        ...
        first_seen_at, last_seen_at, is_super_fan, needs_attention, good_impression, gift_excluded, is_super_lurker, is_blocked, notes, tags,
        host_follow_status, host_follow_status_updated_at,
        profile_follower_count, profile_following_count, profile_counts_updated_at,
        manually_updated_at, created_at, updated_at
      FROM listeners WHERE user_id = $1
  ...
  async updateListener(userId, { isSuperFan, needsAttention, isSuperLurker, isBlocked, notes, tags, goodImpression, giftExcluded } = {}) {
    if (!this.ready) return null;
    const result = await this.pool.query(`
