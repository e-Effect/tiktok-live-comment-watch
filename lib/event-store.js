...
export class EventStore {
  ...
  async listenerContributionRankings({ username = "", fresh = false } = {}) {
    ...
    if (!fresh && cached && cached.generatedAt >= Date.now() - CONTRIBUTION_RANK_CACHE_MS) return cached;
    ...
    const result = await this.pool.query(`
      WITH lifetime AS (
        ...
        SELECT user_id,
          ...
          COALESCE(SUM(diamonds) FILTER (WHERE event_type = 'gift'), 0)::bigint AS recent_coins,
      ...
      SELECT l.user_id,
        ...
        t.visits, t.comments, t.coins, t.likes, t.stats_last_seen_at,
      ...
      FROM lifetime t
      JOIN listeners l ON l.user_id = t.user_id
      LEFT JOIN recent_visits rv ON rv.user_id = t.user_id
      LEFT JOIN recent_events re ON re.user_id = t.user_id
    `, [username]);
    const rankings = {
