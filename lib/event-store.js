...
export class EventStore {
  ...
  async recordVisit(session, visit) {
    ...
    try {
      ...
      const heartMeHistory = await this.heartMeHistory({
    ...
    const result = await this.pool.query(`
      WITH totals AS (
        ...
        SELECT l.user_id, l.latest_unique_id, l.latest_nickname, l.avatar_url,
          ...
          l.first_seen_at, l.last_seen_at, l.is_super_fan, l.notes, l.tags,
  ...
  async listenerExportRows({ username = "", search = "" } = {}) {
    ...
    const result = await this.pool.query(`
      ...
      SELECT l.user_id, l.latest_unique_id, l.latest_nickname,
        l.is_super_fan, l.notes, l.tags, t.*
      FROM totals t JOIN listeners l ON l.user_id = t.user_id
      WHERE ($2 = '' OR l.user_id ILIKE '%' || $2 || '%'
        OR l.latest_unique_id ILIKE '%' || $2 || '%'
        OR l.latest_nickname ILIKE '%' || $2 || '%'
  ...
  async listenerDetail(userId, { username = "" } = {}) {
    ...
    const [stats, aliases, gifts, comments, events, stamps, prints] = await Promise.all([
      ...
      this.pool.query(`SELECT event_type AS type, event_at AS "at", comment_text AS text, gift_id AS "giftId", gift_name AS "giftName", item_count AS count, diamonds AS coins, payload->>'giftImageUrl' AS "giftImageUrl", stream_username AS "streamUsername" FROM live_events WHERE user_id = $1 AND ($2 = '' OR LOWER(stream_username) = LOWER($2)) ORDER BY event_at DESC LIMIT 200`, [userId, username]),
