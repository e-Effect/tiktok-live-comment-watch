...
export class EventStore {
  ...
  async listeners({ username = "", search = "", sort = "last_seen", direction = "desc", classification = "all", blocked = "all", giftExperience = "all", commentExperience = "all", limit = 100, offset = 0, fresh = false } = {}) {
      ...
      const fastResult = await this.pool.query(`
        WITH page AS MATERIALIZED (
          SELECT l.user_id, l.latest_unique_id, l.latest_nickname, l.avatar_url,
            ...
            l.first_seen_at, l.last_seen_at, l.is_super_fan, l.needs_attention, l.is_super_lurker, l.is_blocked, l.notes, l.tags,
    ...
    const result = await this.pool.query(`
      ...
      SELECT p.*, l.latest_unique_id, l.latest_nickname, l.avatar_url,
        (l.avatar_data IS NOT NULL) AS avatar_cached,
        l.first_seen_at, l.last_seen_at, l.is_super_fan, l.needs_attention,
  ...
  async refreshListenerContributionRankings(username, cacheKey = String(username || "").trim().toLowerCase() || "*") {
    ...
    const refresh = (async () => {
      const result = await this.pool.query(`
      WITH rankable_gifts AS MATERIALIZED (
        SELECT user_id, SUM(diamonds)::bigint AS coins
        FROM live_events
        WHERE event_type = 'gift'
          AND event_at >= NOW() - INTERVAL '90 days' AND event_at <= NOW()
          AND user_id <> ''
          AND item_count > 0
          AND diamonds::numeric / NULLIF(item_count, 0) > 10
          AND ($1 = '' OR LOWER(stream_username) = LOWER($1))
        GROUP BY user_id HAVING SUM(diamonds) >= 100
        ...
        FROM live_events e JOIN rankable_gifts g ON g.user_id=e.user_id
        WHERE event_at >= NOW() - INTERVAL '90 days' AND event_at <= NOW()
          AND event_type IN ('comment', 'like')
          AND ($1 = '' OR LOWER(e.stream_username) = LOWER($1))
        GROUP BY e.user_id HAVING COUNT(*) FILTER (WHERE event_type='comment') >= 10
      ), recent_visits AS (
        SELECT v.user_id,
          COUNT(DISTINCT COALESCE(NULLIF(s.room_id,''),v.session_id::text))::bigint AS recent_visits,
          MAX(v.last_seen_at) AS recent_visit_at
        FROM viewer_visits v JOIN recent_events e ON e.user_id=v.user_id
          JOIN live_sessions s ON s.id=v.session_id
        WHERE v.last_seen_at >= NOW() - INTERVAL '90 days' AND v.first_seen_at <= NOW()
          AND ($1 = '' OR LOWER(v.stream_username) = LOWER($1))
        GROUP BY v.user_id
      )
      SELECT l.user_id,
  ...
  async listenerExportRows({ username = "", search = "" } = {}) {
    ...
    const result = await this.pool.query(`
      ...
      SELECT l.user_id, l.latest_unique_id, l.latest_nickname,
        l.is_super_fan, l.needs_attention, l.is_super_lurker, l.is_blocked, l.notes, l.tags,
        l.host_follow_status, l.host_follow_status_updated_at,
  ...
  async listenerDetail(userId, { username = "" } = {}) {
    ...
    const listener = await this.pool.query(`
      SELECT user_id, latest_unique_id, latest_nickname, avatar_url,
        ...
        first_seen_at, last_seen_at, is_super_fan, needs_attention, is_super_lurker, is_blocked, notes, tags,
        host_follow_status, host_follow_status_updated_at,
        profile_follower_count, profile_following_count, profile_counts_updated_at,
        manually_updated_at, created_at, updated_at
      FROM listeners WHERE user_id = $1
  ...
  async updateListener(userId, { isSuperFan, needsAttention, isSuperLurker, isBlocked, notes, tags } = {}) {
    if (!this.ready) return null;
    const result = await this.pool.query(`
