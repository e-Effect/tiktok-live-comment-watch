...
export class EventStore {
    ...
    try {
      ...
      await client.query(`
        INSERT INTO listeners (
          ...
          first_seen_at, last_seen_at, is_super_fan, is_super_lurker, notes, tags,
  ...
  async listeners({ username = "", search = "", sort = "last_seen", direction = "desc", classification = "all", limit = 100, offset = 0, fresh = false } = {}) {
    ...
    const fastOrderBy = !username && classificationMode === "all" ? fastSortColumns[sort] : null;
    if (fastOrderBy) {
      ...
      const fastResult = await this.pool.query(`
        WITH page AS MATERIALIZED (
          SELECT l.user_id, l.latest_unique_id, l.latest_nickname, l.avatar_url,
            ...
            l.first_seen_at, l.last_seen_at, l.is_super_fan, l.needs_attention, l.is_super_lurker, l.notes, l.tags,
  ...
  async listenerContributionPage({ username = "", search = "", sort = "contribution", classification = "all", limit = 100, offset = 0, fresh = false } = {}) {
  ...
  async listenerExportRows({ username = "", search = "" } = {}) {
    ...
    const result = await this.pool.query(`
      ...
      SELECT l.user_id, l.latest_unique_id, l.latest_nickname,
        l.is_super_fan, l.needs_attention, l.is_super_lurker, l.notes, l.tags,
        l.host_follow_status, l.host_follow_status_updated_at,
  ...
  async listenerDetail(userId, { username = "" } = {}) {
    ...
    const listener = await this.pool.query(`
      SELECT user_id, latest_unique_id, latest_nickname, avatar_url,
        ...
        first_seen_at, last_seen_at, is_super_fan, needs_attention, is_super_lurker, notes, tags,
        host_follow_status, host_follow_status_updated_at,
        profile_follower_count, profile_following_count, profile_counts_updated_at,
        manually_updated_at, created_at, updated_at
      FROM listeners WHERE user_id = $1
  ...
  async updateListener(userId, { isSuperFan, needsAttention, isSuperLurker, notes, tags } = {}) {
    if (!this.ready) return null;
    const result = await this.pool.query(`
...
function emptyListenerSummary() {
  return { listeners: 0, superFans: 0, avatars: 0, visits: 0, comments: 0, gifts: 0, coins: 0, shares: 0, activeToday: 0, returning: 0, lurkers: 0 };
}
