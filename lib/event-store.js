...
export class EventStore {
  ...
  async listeners({ username = "", search = "", sort = "last_seen", direction = "desc", classification = "all", limit = 100, offset = 0 } = {}) {
    ...
    if (fastOrderBy) {
      const fastResult = await this.pool.query(`
        WITH page AS MATERIALIZED (
          ...
          SELECT COUNT(*)::bigint AS full_count
          FROM listeners l
          WHERE LOWER(l.user_id) <> 'unknown'
            AND ($1 = '' OR LOWER(l.user_id || ' ' || l.latest_unique_id || ' ' || l.latest_nickname || ' ' || l.notes)
            LIKE '%' || LOWER($1) || '%')
        )
        SELECT p.*, t.*, c.full_count
        FROM page p LEFT JOIN totals t ON t.user_id = p.user_id
        CROSS JOIN matched_count c
