...
import { NoonRetentionWindow } from "./noon-retention-window.js";
import { RuntimeDiagnostics } from "./runtime-diagnostics.js";
import { readTrialMetrics } from "./trial-metrics.js";
import { readRankMetrics } from "./rank-metrics.js";
import { buildContributionRankings, noContributionRank } from "./contribution-rank-v2.js";
import { classifyLurker, LURKER_RULES } from "./lurker-classification.js";

const MAX_QUERY_ROWS = 200;
const COMMENT_BODY_LIMIT = 1500;
const COMMENT_PRUNE_BATCH = 250;
const CONTRIBUTION_RANK_CACHE_MS = 6 * 60 * 60 * 1000;
const LISTENER_COUNT_CACHE_MS = 5 * 60 * 1000;
const LISTENER_IDENTITY_CACHE_MAX = 20000;

function delay(milliseconds) {
  return new Promise((resolve) => setTimeout(resolve, milliseconds));
}

function experienceCondition(mode, column) {
  return mode === "yes" ? `COALESCE(${column}, 0) > 0`
    : mode === "no" ? `COALESCE(${column}, 0) = 0` : "TRUE";
}

function matchesExperience(mode, count) {
  return mode === "yes" ? Number(count || 0) > 0 : mode === "no" ? Number(count || 0) === 0 : true;
}

function isRetryableDatabaseMessage(message) {
  return /deadlock detected|serialization failure|could not serialize|lock not available|lock timeout/i.test(String(message || ""));
}

function isRetryableDatabaseError(error) {
  return ["40P01", "40001", "55P03"].includes(String(error?.code || ""))
    || isRetryableDatabaseMessage(error?.message || error);
}
...
export class EventStore {
  constructor({ connectionString = "", ssl } = {}) {
    ...
    this.visitPool = null;
    this.writePool = null;
    this.eventWriteStats = { count: 0, failures: 0, identityMaxMs: 0, writeMaxMs: 0, lastFailurePhase: "" };
    this.visitBatchStats = { batches: 0, users: 0, failures: 0, lastMs: 0, maxMs: 0 };
    this.ready = false;
    this.lastError = "";
    ...
    this.contributionRankRestorePromises = new Map();
    this.visitEnrichmentTail = Promise.resolve();
    this.visitEnrichmentPending = 0;
    this.listenerCountCache = new Map();
    this.listenerCountPromises = new Map();
    this.listenerMergePromises = new Map();
    this.listenerIdentityCache = new Map();
    this.inboxRecovery = { recovered: 0, lastCheckedAt: null, lastError: "" };
    this.commentRetentionQueue = new Set();
    this.commentRetentionSeedAt = 0;
    this.commentRetentionWindow = new NoonRetentionWindow();
    this.commentRetention = { limitPerUser: COMMENT_BODY_LIMIT, prunedBodies: 0, lastCheckedAt: null, lastError: "" };
  }
  ...
  async initialize() {
    // Recover existing pools without draining enrichment or rerunning migrations.
    if (this.schemaReady && this.pool && this.inboxPool) {
      try {
        await this.inboxPool.query("SELECT 1");
        this.ready = true;
        this.lastError = "";
        return true;
      } catch (error) {
    ...
    try {
      ...
      pool = new Pool({
        ...
        query_timeout: 35000,
        application_name: "live-ledger-general"
      });
      pool.on("error", (error) => {
      ...
      this.pool = pool;
      this.writePool = new Pool({connectionString:this.connectionString,
        ssl:useSsl?{rejectUnauthorized:false}:false,max:1,connectionTimeoutMillis:3000,
        statement_timeout:5000,query_timeout:6000,application_name:"live-event-writer"});
      this.writePool.on("error",error=>this.rememberError(error));
      this.visitPool = new Pool({
        connectionString: this.connectionString,
        ssl: useSsl ? { rejectUnauthorized: false } : false,
        max: 1, connectionTimeoutMillis: 3000, statement_timeout: 4000,
        query_timeout: 5000, application_name: "live-visit-judgment"
      });
      this.visitPool.on("error", error => this.rememberError(error));
      // Reserve one connection for the small receipt ACK. Slow ledger reads
      // must not occupy every connection and delay the next collector batch.
      this.inboxPool = new Pool({
        connectionString: this.connectionString,
        ssl: useSsl ? { rejectUnauthorized: false } : false,
        max: 1,
      ...
      this.inboxPool.on("error", error => this.rememberError(error));
      for (const [name, target] of [["general",pool],["writer",this.writePool],["visits",this.visitPool],["inbox",this.inboxPool]]) {
        this.runtimeDiagnostics.instrument(target, name);
      }
      await pool.query(`
        ...
        CREATE INDEX IF NOT EXISTS live_event_inbox_time_idx ON live_event_inbox (received_at);
        ...
        ALTER TABLE live_events ADD COLUMN IF NOT EXISTS comment_body_pruned BOOLEAN NOT NULL DEFAULT FALSE;
        CREATE INDEX IF NOT EXISTS live_events_time_idx
          ON live_events (event_at DESC);
        ...
        CREATE TABLE IF NOT EXISTS listener_birthdays (
          user_id TEXT PRIMARY KEY,
          birthday TEXT NOT NULL,
          pending_birthday TEXT,
          updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
        );
        CREATE TABLE IF NOT EXISTS listener_birthday_celebrations (
          stream_username TEXT NOT NULL, user_id TEXT NOT NULL, day DATE NOT NULL,
          created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          PRIMARY KEY(stream_username,user_id,day)
        );
        ...
        ALTER TABLE listeners ADD COLUMN IF NOT EXISTS welcome_notice BOOLEAN NOT NULL DEFAULT FALSE;
        ALTER TABLE listeners ADD COLUMN IF NOT EXISTS gift_excluded BOOLEAN NOT NULL DEFAULT FALSE;
        ALTER TABLE listeners ADD COLUMN IF NOT EXISTS good_impression BOOLEAN NOT NULL DEFAULT FALSE;
        ALTER TABLE listeners
          ADD COLUMN IF NOT EXISTS is_super_lurker BOOLEAN NOT NULL DEFAULT FALSE;
        ALTER TABLE listeners
        ...
        CREATE INDEX IF NOT EXISTS listeners_first_seen_idx
          ON listeners (first_seen_at ASC);
        CREATE INDEX IF NOT EXISTS listeners_display_name_idx
          ON listeners (LOWER(COALESCE(NULLIF(latest_nickname, ''), NULLIF(latest_unique_id, ''), user_id)) ASC);
        CREATE INDEX IF NOT EXISTS listeners_super_fan_revision_idx
          ON listeners (manually_updated_at DESC)
  ...
  status() {
    return {
      ...
      ready: this.ready,
      inboxRecovery: this.inboxRecovery,
      commentRetention: { ...this.commentRetention, pendingUsers: this.commentRetentionQueue.size, schedule: "12:00 Asia/Tokyo", catchUp: false, stopAt: "12:15 Asia/Tokyo" },
      recoveryPolicy: "probe-existing-pools-v1",
      diagnosticsPolicy: "metadata-only-no-db-poll-v1",
      eventLockPolicy: "listener-alias-stats-v1",
      eventWriteStats: this.eventWriteStats,
      pools: Object.fromEntries([['general',this.pool],['visits',this.visitPool],['writer',this.writePool],['inbox',this.inboxPool]].map(([name,pool])=>[name,{total:pool?.totalCount||0,idle:pool?.idleCount||0,waiting:pool?.waitingCount||0}])),
  ...
  // A small append-only inbox is committed before the PC discards its copy.
  // Expensive listener/statistics work is deliberately not part of this ACK.
  async stageEvents(session, events) {
    if (!this.ready) return false;
    if (!events.length) return true;
    const sessionData = { id: session.id, username: session.username, startedAt: session.startedAt,
      roomId: session.roomId || "", provider: session.provider || "collector" };
    try {
      await (this.inboxPool || this.pool).query(`
        INSERT INTO live_event_inbox (event_key, session_data, event_data)
        SELECT item->>'key', $1::jsonb, item->'event'
        FROM jsonb_array_elements($2::jsonb) item
        ON CONFLICT (event_key) DO NOTHING
      `, [JSON.stringify(sessionData), JSON.stringify(events.map(event => ({
        key: `${session.id}:${event.id}`, event
      })))]);
      return true;
    } catch (error) {
      this.rememberError(error);
  ...
  async pendingInbox(limit = 100, excludedSessionIds = []) {
    if (!this.ready) return [];
    const result = await this.pool.query(`
      SELECT session_data, event_data FROM live_event_inbox
      WHERE NOT (session_data->>'id' = ANY($2::text[]))
      ORDER BY received_at, event_key LIMIT $1
    `, [Math.min(1000, Math.max(1, limit)), excludedSessionIds]);
    return result.rows.map(row => ({ session: row.session_data, event: row.event_data }));
  }

  async recoverInbox(excludedSessionIds = []) {
    // Replays persistence only. Never replay SSE, slots, stamps or printing.
    // Active sessions already own their queued items and are excluded.
    try {
      const pending = await this.pendingInbox(100, excludedSessionIds);
      const ensured = new Set();
      for (const { session, event } of pending) {
        if (!ensured.has(session.id)) {
          await this.pool.query(`
            INSERT INTO live_sessions (id, stream_username, started_at, status, auto_resume)
            VALUES ($1, $2, $3, 'stopped', FALSE) ON CONFLICT (id) DO NOTHING
          `, [session.id, session.username, new Date(session.startedAt)]);
          ensured.add(session.id);
        }
        if (!await this.recordEvent(session, event)) {
          this.inboxRecovery.lastCheckedAt = Date.now();
          this.inboxRecovery.lastError = this.lastError || "Inbox replay is waiting for persistence";
          return;
        }
        this.inboxRecovery.recovered += 1;
      }
      this.inboxRecovery.lastCheckedAt = Date.now();
      this.inboxRecovery.lastError = "";
    } catch (error) {
      this.inboxRecovery.lastError = String(error?.message || error).slice(0, 200);
    }
  }

  async recordEvent(session, event) {
    if (!this.ready || !event?.type || !event?.id) return false;
    // An identity-less TikFinity member notification is a transport event, not
    // a listener. Treat it as consumed so it is not retried into the database.
    if (isAnonymousListenerIdentity(event)) return true;
    let phase = "identity";
    let phaseAt = Date.now();
    try {
      event.userId = await this.resolveListenerId(event.userId, event.uniqueId);
      this.eventWriteStats.identityMaxMs = Math.max(this.eventWriteStats.identityMaxMs,Date.now()-phaseAt);
      phase = "write";
      phaseAt = Date.now();
      const payload = event.payload && typeof event.payload === "object" ? { ...event.payload } : {};
      if (event.giftImageUrl) payload.giftImageUrl = String(event.giftImageUrl).slice(0, 2000);
      for (const field of ["collectorReceivedAt", "collectorQueuedAt", "serverReceivedAt", "durableAt", "persistenceStartedAt"]) {
      ...
      await (this.writePool || this.pool).query(`
        WITH inserted AS (
          ...
          DELETE FROM live_event_inbox WHERE event_key = $1
        ), upsert_listener AS (
          ...
          FROM inserted WHERE user_id <> ''
            AND EXISTS (SELECT 1 FROM upsert_listener)
        ...
        FROM inserted WHERE user_id <> ''
          AND EXISTS (SELECT 1 FROM upsert_alias)
      ...
      delete event.__databaseRetryCount;
      if (event.type === "comment" && event.userId) this.commentRetentionQueue.add(String(event.userId));
      this.eventWriteStats.count += 1;
      this.eventWriteStats.writeMaxMs = Math.max(this.eventWriteStats.writeMaxMs,Date.now()-phaseAt);
      this.clearTransientError();
      return true;
    } catch (error) {
      this.eventWriteStats.failures += 1;
      this.eventWriteStats.lastFailurePhase = phase;
      const retryCount = Math.max(0, Number(event.__databaseRetryCount || 0));
      if (isRetryableDatabaseError(error) && retryCount < 3) {
        event.__databaseRetryCount = retryCount + 1;
        await delay(40 * (2 ** retryCount) + Math.floor(Math.random() * 30));
        return this.recordEvent(session, event);
      }
      delete event.__databaseRetryCount;
      this.rememberError(error);
      return false;
  ...
  async resolveListenerId(rawUserId, rawUniqueId) {
    ...
    const identityKey = `${userId}\u0000${uniqueId.toLowerCase()}`;
    if (this.listenerIdentityCache.has(identityKey)) return userId;
    const match = await (this.writePool || this.pool).query(`
    ...
    if (sourceId && sourceId !== userId) {
      const mergeKey = `${sourceId}->${userId}`;
      let mergePromise = this.listenerMergePromises.get(mergeKey);
      if (!mergePromise) {
        mergePromise = this.mergeListenerRecords(sourceId, userId)
          .finally(() => this.listenerMergePromises.delete(mergeKey));
        this.listenerMergePromises.set(mergeKey, mergePromise);
      }
      await mergePromise;
    }
    this.listenerIdentityCache.set(identityKey, true);
    if (this.listenerIdentityCache.size > LISTENER_IDENTITY_CACHE_MAX) {
      const oldestKey = this.listenerIdentityCache.keys().next().value;
      if (oldestKey !== undefined) this.listenerIdentityCache.delete(oldestKey);
    }
    return userId;
  }

  clearTransientError() {
    if (isRetryableDatabaseMessage(this.lastError)) this.lastError = "";
  }
    ...
    try {
      ...
      await client.query(`
        INSERT INTO listeners (
          ...
          first_seen_at, last_seen_at, is_super_fan, is_super_lurker, is_blocked, notes, tags,
        ...
        ON CONFLICT (user_id) DO UPDATE SET
          ...
          is_super_lurker = listeners.is_super_lurker OR EXCLUDED.is_super_lurker,
          is_blocked = listeners.is_blocked OR EXCLUDED.is_blocked,
          host_follow_status = CASE
            WHEN EXCLUDED.host_follow_status_updated_at IS NOT NULL
              AND (listeners.host_follow_status_updated_at IS NULL OR EXCLUDED.host_follow_status_updated_at > listeners.host_follow_status_updated_at)
            THEN EXCLUDED.host_follow_status ELSE listeners.host_follow_status END,
  ...
  async recordVisitBatch(session, visits) {
    if (!this.ready || !visits.length) return new Map();
    const pool = this.visitPool || this.pool;
    const started = Date.now();
    this.visitBatchStats.batches += 1;
    this.visitBatchStats.users += visits.length;
    try {
      // Stable ordering prevents overlapping reconnect batches taking row locks in opposite order.
      await pool.query(`INSERT INTO viewer_visits
        (session_id,stream_username,user_id,nickname,first_seen_at,last_seen_at,first_source)
        SELECT $1,$2,x."userId",x.nickname,to_timestamp(x.at/1000.0),to_timestamp(x.at/1000.0),x.source
        FROM jsonb_to_recordset($3::jsonb) AS x("userId" text,nickname text,at double precision,source text)
        ORDER BY x."userId"
        ON CONFLICT (session_id,user_id) DO UPDATE SET nickname=EXCLUDED.nickname,
          last_seen_at=GREATEST(viewer_visits.last_seen_at,EXCLUDED.last_seen_at)`,
        [session.id,session.username,JSON.stringify(visits)]);
      const result = await pool.query(`SELECT v.user_id AS "userId",
        COUNT(DISTINCT COALESCE(NULLIF(s.room_id,''),v.session_id::text))::bigint AS "visitCount",
        MIN(v.first_seen_at) AS "firstVisitAt",MAX(v.last_seen_at) AS "lastVisitAt",
        MAX(v.last_seen_at) FILTER (WHERE COALESCE(NULLIF(s.room_id,''),v.session_id::text)<>$3) AS "previousVisitAt"
        FROM viewer_visits v JOIN live_sessions s ON s.id=v.session_id
        WHERE v.user_id=ANY($2::text[]) AND LOWER(v.stream_username)=LOWER($1)
        GROUP BY v.user_id`,[session.username,visits.map(v=>String(v.userId)),String(session.roomId||session.id)]);
      return new Map(result.rows.map(row=>[row.userId,{
        visitHistoryKnown:true,visitCount:Number(row.visitCount),
        firstVisitAt:row.firstVisitAt?new Date(row.firstVisitAt).getTime():null,
        lastVisitAt:row.lastVisitAt?new Date(row.lastVisitAt).getTime():null,
        previousVisitAt:row.previousVisitAt?new Date(row.previousVisitAt).getTime():null
      }]));
    } catch(error) {
      this.visitBatchStats.failures += 1;
      this.rememberError(error);
      return new Map();
    } finally {
      this.visitBatchStats.lastMs=Date.now()-started;
      this.visitBatchStats.maxMs=Math.max(this.visitBatchStats.maxMs,this.visitBatchStats.lastMs);
    }
  }

  async recordVisit(session, visit, options = {}) {
    const fallback = {
    ...
    if (!this.ready || !visit?.userId || isAnonymousListenerIdentity(visit)) return fallback;
    try {
      const at = new Date(visit.at || Date.now());
      const currentVisitKey = String(session.roomId || session.id);
      let row = options.precomputed;
      if (!row) {
      await (this.visitPool || this.pool).query(`
      ...
      const result = await (this.visitPool || this.pool).query(`
        SELECT
          COUNT(DISTINCT COALESCE(NULLIF(s.room_id, ''), v.session_id::text))::bigint AS "visitCount",
      ...
      if (typeof options.onJudgment === "function") {
        await options.onJudgment(visitJudgment);
      }
      const enrich = async () => {
      ...
      const [heartMeResult, listenerSyncResult] = await Promise.allSettled([heartMeHistoryPromise, listenerSyncPromise]);
      if (listenerSyncResult.status === "rejected") this.rememberError(listenerSyncResult.reason);
      const heartMeHistory = heartMeResult.status === "fulfilled"
        ? heartMeResult.value
        : { known: false, pastCount: 0, lastAt: null };
      return {
        ...visitJudgment,
      ...
      if (options.deferEnrichment) {
        // Do not hold up the next visit judgment for these auxiliary lookups.
        // A single shared tail bounds their database concurrency across sessions.
        this.visitEnrichmentPending += 1;
        this.visitEnrichmentTail = this.visitEnrichmentTail.then(enrich)
          .then((summary) => options.onEnriched?.(summary))
          .catch((error) => this.rememberError(error))
          .finally(() => { this.visitEnrichmentPending -= 1; });
        return visitJudgment;
      }
      return await enrich();
    } catch (error) {
      this.rememberError(error);
  ...
  async priorListenerHistory({ sessionId = "", roomId = "", username = "", userId = "", uniqueId = "" } = {}) {
    ...
    try {
      ...
      const result = await this.pool.query(`
        WITH identity_ids AS (
          ...
          SELECT COALESCE(NULLIF(s.room_id, ''), v.session_id::text) AS live_key,
                 v.last_seen_at AS seen_at, TRUE AS is_visit, FALSE AS is_action
          FROM viewer_visits v
          JOIN live_sessions s ON s.id = v.session_id
          WHERE LOWER(v.stream_username) = LOWER($1)
          ...
          SELECT COALESCE(NULLIF(s.room_id, ''), e.session_id::text) AS live_key,
                 e.event_at AS seen_at, FALSE AS is_visit, TRUE AS is_action
          FROM live_events e
          JOIN live_sessions s ON s.id = e.session_id
          WHERE LOWER(e.stream_username) = LOWER($1)
            AND e.user_id IN (SELECT user_id FROM identity_ids)
            AND e.event_type IN ('like', 'comment', 'gift')
            AND COALESCE(NULLIF(s.room_id, ''), e.session_id::text) <> $4
        ), prior_lives AS (
          SELECT live_key, MAX(seen_at) AS seen_at, BOOL_OR(is_visit) AS is_visit, BOOL_OR(is_action) AS is_action
          FROM prior_records
          GROUP BY live_key
        )
        SELECT COUNT(*) FILTER (WHERE is_visit)::bigint AS "priorVisitCount",
               COALESCE(BOOL_OR(is_action), FALSE) AS "hasPriorInteraction",
      ...
      return {
        ...
        hasPriorInteraction: Boolean(row.hasPriorInteraction),
  ...
  async listenerSummary({ username = "" } = {}) {
    ...
    const result = await this.pool.query(`
      WITH profiles AS MATERIALIZED (
        SELECT user_id, is_super_fan, is_blocked,
          (avatar_data IS NOT NULL) AS has_avatar
        FROM listeners WHERE LOWER(user_id) <> 'unknown'
      ...
      SELECT
        ...
        COUNT(*) FILTER (WHERE l.is_blocked)::bigint AS blocked,
        COUNT(*) FILTER (WHERE l.has_avatar)::bigint AS avatars,
        ...
        COUNT(*) FILTER (WHERE ${lurkerSqlCondition("t")})::bigint AS lurkers
      FROM totals t JOIN profiles l ON l.user_id = t.user_id
    `, [username, rangeStart("today")]);
    return normalizeSummaryRow(result.rows[0]);
  }

  async listeners({ username = "", search = "", sort = "last_seen", direction = "desc", classification = "all", blocked = "all", attention = "all", welcome = "all", giftExperience = "all", commentExperience = "all", limit = 100, offset = 0, fresh = false } = {}) {
    ...
    const classificationMode = normalizeListenerClassification(classification);
    const blockedMode = normalizeListenerBlockedFilter(blocked);
    const fastSortColumns = {
      last_seen: "last_seen_at",
      first_seen: "first_seen_at",
      name: "display_name_sort"
    };
    const experienceFiltered = [giftExperience, commentExperience].some(v => v === "yes" || v === "no");
    const attentionFiltered = [attention, welcome].some(value => value === "yes" || value === "no");
    const fastOrderBy = !attentionFiltered && !username && !experienceFiltered && classificationMode === "all" && blockedMode === "all" ? fastSortColumns[sort] : null;
    if (fastOrderBy) {
      // Fetch the visible page before starting the expensive all-listener
      // count. Running both together made the small production database fight
      // for I/O and delayed the rows the user was actually waiting to see.
      const fastResult = await this.pool.query(`
        WITH page AS MATERIALIZED (
          SELECT l.user_id, l.latest_unique_id, l.latest_nickname, l.avatar_url,
            ...
            l.first_seen_at, l.last_seen_at, l.is_super_fan, l.needs_attention, l.good_impression, l.gift_excluded, l.welcome_notice, l.is_super_lurker, l.is_blocked, l.notes, l.tags,
          ...
          FROM listeners l
          WHERE LOWER(l.user_id) <> 'unknown'
            AND ($1 = '' OR LOWER(l.user_id || ' ' || l.latest_unique_id || ' ' || l.latest_nickname || ' ' || l.notes)
            LIKE '%' || LOWER($1) || '%')
      ...
      `, [query, boundedLimit + 1, boundedOffset]);
      const countResult = await this.listenerCount({ search: query, fresh, waitForRefresh: false });
      const hasMore = fastResult.rows.length > boundedLimit;
      const pageRows = fastResult.rows.slice(0, boundedLimit);
      const lowerBound = boundedOffset + pageRows.length + (hasMore ? 1 : 0);
      return {
        items: pageRows.map(normalizeListenerRow),
        total: countResult.total ?? lowerBound,
        totalPending: Boolean(countResult.pending),
        hasMore
      };
    }
    const result = await this.pool.query(`
      WITH matched_listeners AS MATERIALIZED (
        ...
        FROM listeners l
        WHERE LOWER(l.user_id) <> 'unknown'
          AND ($2 = '' OR LOWER(l.user_id || ' ' || l.latest_unique_id || ' ' || l.latest_nickname || ' ' || l.notes)
          LIKE '%' || LOWER($2) || '%')
          AND ${blockedSqlCondition(blockedMode, "l")}
          AND ${attention === "yes" ? "l.needs_attention = TRUE" : attention === "no" ? "l.needs_attention = FALSE" : "TRUE"}
          AND ${welcome === "yes" ? "l.welcome_notice = TRUE" : welcome === "no" ? "l.welcome_notice = FALSE" : "TRUE"}
      ), totals AS (
        SELECT s.user_id,
          SUM(visit_count)::bigint AS visits,
          SUM(comment_count)::bigint AS comments,
        ...
        SELECT t.*, m.display_name_sort,
          (${lurkerSqlCondition("t")}) AS is_lurker,
          ROUND(t.comments::numeric / GREATEST(t.visits, 1), 2) AS comments_per_visit,
          ROUND(t.coins::numeric / GREATEST(t.visits, 1), 2) AS all_coins_per_visit,
          ROUND(t.visits::numeric / (1 + t.comments + t.coins::numeric / 10), 2) AS lurker_score
        FROM totals t JOIN matched_listeners m ON m.user_id = t.user_id
      ), filtered AS (
        SELECT c.*,
          COUNT(*) OVER()::bigint AS full_count
        FROM classified c
        WHERE ${classificationSqlCondition(classificationMode, "c")}
          AND ${experienceCondition(giftExperience, "c.gifts")}
          AND ${experienceCondition(commentExperience, "c.comments")}
      ), page AS MATERIALIZED (
        SELECT * FROM filtered
        ORDER BY ${orderBy} ${orderDirection} NULLS LAST, stats_last_seen_at DESC, user_id
        LIMIT $3 OFFSET $4
      )
      SELECT p.*, l.latest_unique_id, l.latest_nickname, l.avatar_url,
        (l.avatar_data IS NOT NULL) AS avatar_cached,
        l.first_seen_at, l.last_seen_at, l.is_super_fan, l.needs_attention, l.good_impression, l.gift_excluded, l.welcome_notice,
        l.is_super_lurker, l.is_blocked, l.notes, l.tags,
        l.host_follow_status, l.host_follow_status_updated_at,
        l.profile_follower_count, l.profile_following_count, l.profile_counts_updated_at,
        l.manually_updated_at, l.created_at, l.updated_at
      FROM page p JOIN listeners l ON l.user_id = p.user_id
      ORDER BY p.${orderBy} ${orderDirection} NULLS LAST, p.stats_last_seen_at DESC, p.user_id
  ...
  async listenerContributionRankings({ username = "", fresh = false, waitForRefresh = true } = {}) {
    if (!this.ready) return emptyContributionRankings();
    const cacheKey = String(username || "").trim().toLowerCase() || "*";
    let cached = this.contributionRankCache.get(cacheKey);
    if (!cached) cached = await this.restoreContributionRankings(cacheKey);
    // Normal ledger pages and listener details must never launch the expensive
    // all-listener ranking query. Ranking is refreshed only by an explicit
    // contribution-sort request, where waitForRefresh remains true.
    if (!waitForRefresh) return cached || emptyContributionRankings();
    if (!fresh && cached) {
  ...
  async restoreContributionRankings(cacheKey) {
    if (!this.contributionRankRestorePromises.has(cacheKey)) {
      const restore = (async () => {
        const result = await this.pool.query(
          "SELECT state FROM shared_app_states WHERE state_key = $1",
          ["contribution-ranks:v2-90d:" + cacheKey]
        );
        const saved = result.rows[0]?.state;
        if (!saved || !Array.isArray(saved.entries) || !Array.isArray(saved.lifetimeOrder) || !Array.isArray(saved.recentOrder)) return null;
        const restored = {byUserId:new Map(saved.entries), lifetimeOrder:saved.lifetimeOrder, recentOrder:saved.recentOrder, generatedAt:Number(saved.generatedAt)||0};
        if (!this.contributionRankCache.has(cacheKey)) this.contributionRankCache.set(cacheKey,restored);
        return this.contributionRankCache.get(cacheKey);
      })();
      this.contributionRankRestorePromises.set(cacheKey,restore);
      restore.catch(()=>this.contributionRankRestorePromises.delete(cacheKey));
    }
    return this.contributionRankRestorePromises.get(cacheKey);
  }

  async listenerCount({ search = "", fresh = false, waitForRefresh = true } = {}) {
    if (!this.ready) return { total: 0, pending: false };
    const normalizedSearch = String(search || "").trim().toLocaleLowerCase("ja-JP");
    const cacheKey = normalizedSearch || "*";
    const cached = this.listenerCountCache.get(cacheKey);
    if (cached && !fresh) {
      if (cached.generatedAt < Date.now() - LISTENER_COUNT_CACHE_MS) {
        this.refreshListenerCount(normalizedSearch, cacheKey).catch(() => {});
      }
      return { total: cached.total, pending: false };
    }
    if (cached && !waitForRefresh) {
      this.refreshListenerCount(normalizedSearch, cacheKey).catch(() => {});
      return { total: cached.total, pending: false };
    }
    const refresh = this.refreshListenerCount(normalizedSearch, cacheKey);
    if (!waitForRefresh) {
      refresh.catch(() => {});
      return { total: cached?.total ?? null, pending: !cached };
    }
    return refresh;
  }

  async refreshListenerCount(search, cacheKey = String(search || "").trim().toLocaleLowerCase("ja-JP") || "*") {
    const pending = this.listenerCountPromises.get(cacheKey);
      ...
      const result = await this.pool.query(`
        ...
        WHERE LOWER(l.user_id) <> 'unknown'
          AND ($1 = '' OR LOWER(l.user_id || ' ' || l.latest_unique_id || ' ' || l.latest_nickname || ' ' || l.notes)
            LIKE '%' || LOWER($1) || '%')
      `, [search]);
      const value = { total: Number(result.rows[0]?.total || 0), pending: false, generatedAt: Date.now() };
      this.listenerCountCache.set(cacheKey, value);
      return value;
    })();
    this.listenerCountPromises.set(cacheKey, refresh);
    try {
      return await refresh;
    } finally {
      if (this.listenerCountPromises.get(cacheKey) === refresh) this.listenerCountPromises.delete(cacheKey);
    }
  }
  ...
  async refreshListenerContributionRankings(username, cacheKey = String(username || "").trim().toLowerCase() || "*") {
    ...
    if (pending) return pending;
    const refresh = (async () => {
      const rows = await readRankMetrics(this.pool, username);
      const rankings = {
        ...buildContributionRankings(rows),
        generatedAt: Date.now()
      };
      await this.pool.query(`
        INSERT INTO shared_app_states (state_key,state,source,updated_at)
        VALUES ($1,$2::jsonb,'contribution-ranking',NOW())
        ON CONFLICT (state_key) DO UPDATE SET state=EXCLUDED.state,updated_at=NOW()
      `,["contribution-ranks:v2-90d:" + cacheKey,JSON.stringify({entries:[...rankings.byUserId],lifetimeOrder:rankings.lifetimeOrder,recentOrder:rankings.recentOrder,generatedAt:rankings.generatedAt})]);
  ...
  async listenerContributionForIds({ username = "", userIds = [], fresh = false } = {}) {
    const rankings = await this.listenerContributionRankings({ username, fresh, waitForRefresh: false });
    return new Map(userIds.map((userId) => [String(userId), publicContributionRank(rankings.byUserId.get(String(userId)) || noContributionRank(Boolean(rankings.generatedAt)))]));
  }

  async listenerContributionPage({ username = "", search = "", sort = "contribution", classification = "all", blocked = "all", attention = "all", welcome = "all", giftExperience = "all", commentExperience = "all", limit = 100, offset = 0, fresh = false } = {}) {
    let timer;
    const refresh = this.listenerContributionRankings({ username, fresh });
    refresh.catch(() => {});
    const rankings = await Promise.race([refresh, new Promise(resolve => {
      timer = setTimeout(() => resolve({...emptyContributionRankings(), pending:true}), 1000);
    })]).finally(() => clearTimeout(timer));
    if (rankings.pending) return {items:[], total:0, rankingGeneratedAt:0, rankingPending:true};
    ...
    let filteredIds = normalizedSearch
      ? order.filter((userId) => rankings.byUserId.get(userId)?.searchText.includes(normalizedSearch))
      : order;
    if (welcome === "yes" || welcome === "no") {
      const welcomeIds = new Set(await this.welcomeListenerIds());
      filteredIds = filteredIds.filter(id => welcome === "yes" ? welcomeIds.has(id) : !welcomeIds.has(id));
    }
    if (attention === "yes" || attention === "no") {
      const attentionIds = new Set(await this.attentionListenerIds());
      filteredIds = filteredIds.filter(id => attention === "yes" ? attentionIds.has(id) : !attentionIds.has(id));
    }
    const classificationMode = normalizeListenerClassification(classification);
    if (classificationMode !== "all") {
      const lurkerIds = await this.listenerLurkerIds({ username });
      filteredIds = filteredIds.filter((userId) => classificationMode === "lurker" ? lurkerIds.has(userId) : !lurkerIds.has(userId));
    }
    const blockedMode = normalizeListenerBlockedFilter(blocked);
    if (blockedMode !== "all") {
      const blockedIds = await this.listenerBlockedIds();
      filteredIds = filteredIds.filter((userId) => blockedMode === "blocked" ? blockedIds.has(userId) : !blockedIds.has(userId));
    }
    if ([giftExperience, commentExperience].some(v => v === "yes" || v === "no")) {
      // Ranking excludes cheap gifts; experience must instead use all gift counts.
      const result = await this.pool.query(`SELECT user_id,
        SUM(gift_count)::bigint AS gifts, SUM(comment_count)::bigint AS comments
        FROM listener_stream_stats
        WHERE ($1 = '' OR LOWER(stream_username) = LOWER($1))
        GROUP BY user_id`, [username]);
      const counts = new Map(result.rows.map(row => [String(row.user_id), row]));
      filteredIds = filteredIds.filter(id => {
        const row = counts.get(id) || {};
        return matchesExperience(giftExperience, row.gifts) && matchesExperience(commentExperience, row.comments);
      });
    }
    const boundedLimit = Math.min(250, Math.max(1, Number(limit || 100)));
    const boundedOffset = Math.max(0, Number(offset || 0));
  ...
  async listenerLurkerIds({ username = "" } = {}) {
    ...
    const result = await this.pool.query(`
      SELECT user_id,
        ...
        SUM(gift_coins)::bigint AS coins
      FROM listener_stream_stats
      WHERE LOWER(user_id) <> 'unknown'
        AND ($1 = '' OR LOWER(stream_username) = LOWER($1))
      GROUP BY user_id
    `, [username]);
    return new Set(result.rows.filter((row) => classifyLurker(row).isLurker).map((row) => String(row.user_id)));
  }

  async listenerBlockedIds() {
    if (!this.ready) return new Set();
    const result = await this.pool.query(`SELECT user_id FROM listeners WHERE is_blocked = TRUE`);
  ...
  async listenerExportRows({ username = "", search = "" } = {}) {
    ...
    const result = await this.pool.query(`
      ...
      SELECT l.user_id, l.latest_unique_id, l.latest_nickname,
        l.is_super_fan, l.needs_attention, l.good_impression, l.gift_excluded, l.welcome_notice, l.is_super_lurker, l.is_blocked, l.notes, l.tags,
        l.host_follow_status, l.host_follow_status_updated_at,
        l.profile_follower_count, l.profile_following_count, l.profile_counts_updated_at, t.*
      FROM totals t JOIN listeners l ON l.user_id = t.user_id
      WHERE LOWER(l.user_id) <> 'unknown'
        AND ($2 = '' OR l.user_id ILIKE '%' || $2 || '%'
  ...
  async listenerTrialMetrics(options) {
    if (!this.ready) throw new Error('DBを確認できません');
    return readTrialMetrics(this.pool, options);
  }
  ...
  async listenerDetail(userId, { username = "" } = {}) {
    ...
    const listener = await this.pool.query(`
      SELECT user_id, latest_unique_id, latest_nickname, avatar_url,
        ...
        first_seen_at, last_seen_at, is_super_fan, needs_attention, good_impression, gift_excluded, welcome_notice, is_super_lurker, is_blocked, notes, tags,
        host_follow_status, host_follow_status_updated_at,
        profile_follower_count, profile_following_count, profile_counts_updated_at,
        manually_updated_at, created_at, updated_at
      FROM listeners WHERE user_id = $1
  ...
  async maintainCommentRetention({ blocked = false, now = Date.now } = {}) {
    if (!this.commentRetentionWindow.allow(now(), blocked || !this.ready)) return;
    try {
      if (Date.now() - this.commentRetentionSeedAt > 6 * 60 * 60 * 1000) {
        const candidates = await this.pool.query(`SELECT user_id FROM listener_stream_stats
          GROUP BY user_id HAVING SUM(comment_count) > $1`, [COMMENT_BODY_LIMIT]);
        for (const row of candidates.rows) this.commentRetentionQueue.add(String(row.user_id));
        this.commentRetentionSeedAt = Date.now();
      }
      // Small batches on the existing maintenance loop, never on the receive path.
      for (const userId of [...this.commentRetentionQueue].slice(0, 2)) {
        if (!this.commentRetentionWindow.allow(now(), blocked || !this.ready)) break;
        const result = await (this.writePool || this.pool).query(`
          WITH excess AS (
            SELECT event_key FROM live_events
            WHERE user_id = $1 AND event_type = 'comment' AND NOT comment_body_pruned
            ORDER BY event_at DESC, event_key DESC
            OFFSET $2 LIMIT $3
          )
          UPDATE live_events e SET comment_text = '', payload = '{}'::jsonb, comment_body_pruned = TRUE
          FROM excess WHERE e.event_key = excess.event_key
        `, [userId, COMMENT_BODY_LIMIT, COMMENT_PRUNE_BATCH]);
        this.commentRetentionQueue.delete(userId);
        if (result.rowCount === COMMENT_PRUNE_BATCH) this.commentRetentionQueue.add(userId);
        this.commentRetention.prunedBodies += Number(result.rowCount || 0);
      }
      this.commentRetention.lastCheckedAt = Date.now();
      this.commentRetention.lastError = "";
      if (!this.commentRetentionQueue.size) this.commentRetentionWindow.finish();
    } catch (error) {
      // Retain queue entries for retry; do not interrupt ingest or reset pools.
      this.commentRetention.lastError = shortError(error);
    }
  }
  ...
  async updateListener(userId, { isSuperFan, needsAttention, isSuperLurker, isBlocked, notes, tags, goodImpression, giftExcluded, welcomeNotice } = {}) {
    if (!this.ready) return null;
    const result = await this.pool.query(`
      UPDATE listeners SET
        is_super_fan = COALESCE($2::boolean, is_super_fan),
        needs_attention = COALESCE($3::boolean, needs_attention),
        is_super_lurker = COALESCE($4::boolean, is_super_lurker),
        is_blocked = COALESCE($5::boolean, is_blocked),
        notes = COALESCE($6::text, notes),
        tags = COALESCE($7::jsonb, tags),
        good_impression = COALESCE($8::boolean, good_impression),
        gift_excluded = COALESCE($9::boolean, gift_excluded),
        welcome_notice = COALESCE($10::boolean, welcome_notice),
    ...
    `, [
      ...
      typeof isSuperLurker === "boolean" ? isSuperLurker : null,
      ...
      typeof goodImpression === "boolean" ? goodImpression : null,
      typeof giftExcluded === "boolean" ? giftExcluded : null,
      typeof welcomeNotice === "boolean" ? welcomeNotice : null
    ]);
    return result.rows[0] ? normalizeListenerRow(result.rows[0]) : null;
  }

  async welcomeListenerIds() {
    const result = await this.pool.query("SELECT user_id FROM listeners WHERE welcome_notice = TRUE");
    return result.rows.map(row => row.user_id);
  }

  async attentionListenerIds() {
    const result = await this.pool.query('SELECT user_id FROM listeners WHERE needs_attention = TRUE');
    return result.rows.map(row => row.user_id);
  }

  async listenerManagementFlags({ userId = "", uniqueId = "" } = {}) {
    if (!this.ready) return { known: false, userId: "", uniqueId: "", nickname: "", avatarUrl: "", avatarCached: false, isSuperLurker: false };
    const numericId = String(userId || "").trim();
    const handle = String(uniqueId || "").replace(/^@/, "").trim();
    if ((!numericId && !handle) || isAnonymousListenerIdentity({ userId: numericId, uniqueId: handle })) {
      return { known: false, userId: "", uniqueId: "", nickname: "", avatarUrl: "", avatarCached: false, isSuperLurker: false };
    }
    const result = await this.pool.query(`
      WITH identity_ids AS (
        SELECT $1::text AS user_id WHERE $1 <> ''
        UNION
        SELECT user_id FROM listeners
        WHERE $2 <> '' AND LOWER(latest_unique_id) = LOWER($2)
        UNION
        SELECT user_id FROM listener_aliases
        WHERE $2 <> '' AND LOWER(unique_id) = LOWER($2)
      )
      SELECT user_id AS "userId", latest_unique_id AS "uniqueId", latest_nickname AS nickname,
        avatar_url AS "avatarUrl", (avatar_data IS NOT NULL) AS "avatarCached",
        is_super_lurker AS "isSuperLurker"
      FROM listeners
      WHERE user_id IN (SELECT user_id FROM identity_ids)
    ...
    return {
      ...
      avatarCached: Boolean(row?.avatarCached),
      isSuperLurker: Boolean(row?.isSuperLurker),
    };
  }
  ...
  async superFans() {
    ...
    const result = await this.pool.query(`
      ...
      FROM listeners
      WHERE is_super_fan = TRUE
      ORDER BY last_seen_at DESC NULLS LAST, latest_nickname, latest_unique_id, user_id
    `);
    return result.rows;
  }
  ...
  async recentListenerEvents({ username = "", since = 0, limit = 100 } = {}) {
    ...
    const result = await this.pool.query(`
      SELECT event_key AS id, event_type AS type, event_at AS "at", user_id AS "userId",
        ...
        (payload->>'slotExcluded' = 'true') AS "slotExcluded",
  ...
  async listenerBirthday(userId) {
    if (!this.ready) throw new Error("Database unavailable");
    return (await this.pool.query('SELECT birthday, pending_birthday AS "pendingBirthday" FROM listener_birthdays WHERE user_id=$1',[userId])).rows[0] || {birthday:"",pendingBirthday:null};
  }

  async claimBirthdayCelebration(username,userId,day) {
    if (!this.ready) return false;
    const result=await (this.writePool || this.pool).query(`
      INSERT INTO listener_birthday_celebrations (stream_username,user_id,day)
      SELECT LOWER($1),user_id,$3::date FROM listener_birthdays
      WHERE user_id=$2 AND birthday=to_char($3::date,'MM-DD')
      ON CONFLICT (stream_username,user_id,day) DO NOTHING RETURNING user_id`,[username,userId,day]);
    return result.rows.length>0;
  }

  async registerBirthday(userId, birthday, manual = false) {
    if (!this.ready) throw new Error("Database unavailable");
    const result=await this.pool.query(`INSERT INTO listener_birthdays (user_id,birthday)
      VALUES ($1,$2) ON CONFLICT (user_id) DO UPDATE SET
      birthday=CASE WHEN $3 THEN EXCLUDED.birthday ELSE listener_birthdays.birthday END,
      pending_birthday=CASE WHEN $3 THEN NULL WHEN listener_birthdays.birthday<>EXCLUDED.birthday THEN EXCLUDED.birthday ELSE listener_birthdays.pending_birthday END,
      updated_at=NOW() RETURNING birthday, pending_birthday AS "pendingBirthday"`,[userId,birthday,manual]);
    return result.rows[0];
  }

  async close() {
    this.ready = false;
    await this.visitEnrichmentTail;
    const visitPool = this.visitPool;
    this.visitPool = null;
    await visitPool?.end().catch(() => {});
    ...
    this.contributionRankRestorePromises.clear();
    this.listenerCountCache.clear();
    this.listenerCountPromises.clear();
    this.listenerIdentityCache.clear();
    const inboxPool = this.inboxPool;
    this.inboxPool = null;
    await inboxPool?.end().catch(() => {});
    if (!this.pool) return;
...
function emptyListenerSummary() {
  return { listeners: 0, superFans: 0, blocked: 0, avatars: 0, visits: 0, comments: 0, gifts: 0, coins: 0, shares: 0, activeToday: 0, returning: 0, lurkers: 0 };
}
...
function normalizeListenerRow(row = {}) {
  ...
  const visits = Number(row.visits || row.visit_count || 0);
  const comments = Number(row.comments || row.comment_count || 0);
  const coins = Number(row.coins || row.gift_coins || 0);
  const lurker = classifyLurker({ visits, comments, coins });
  return {
    ...normalized,
    ...
    goodImpression: Boolean(row.good_impression ?? row.goodImpression),
    giftExcluded: Boolean(row.gift_excluded ?? row.giftExcluded),
    welcomeNotice: Boolean(row.welcome_notice ?? row.welcomeNotice),
    isSuperLurker: Boolean(row.is_super_lurker ?? row.isSuperLurker),
...
function normalizeListenerClassification(value) {
  return value === "lurker" || value === "non_lurker" ? value : "all";
}

function normalizeListenerBlockedFilter(value) {
  return value === "blocked" || value === "unblocked" ? value : "all";
}

function emptyContributionRankings() {
  return { byUserId: new Map(), lifetimeOrder: [], recentOrder: [], generatedAt: 0 };
}

function lurkerSqlCondition(alias) {
  return `${alias}.visits >= ${LURKER_RULES.minVisits}
    AND ${alias}.comments::numeric / GREATEST(${alias}.visits, 1) < ${LURKER_RULES.maxCommentsPerVisit}
    AND ${alias}.coins::numeric / GREATEST(${alias}.visits, 1) < ${LURKER_RULES.maxCoinsPerVisit}`;
}

function classificationSqlCondition(mode, alias) {
  if (mode === "lurker") return `${alias}.is_lurker = TRUE`;
  if (mode === "non_lurker") return `${alias}.is_lurker = FALSE`;
  return "TRUE";
}

function blockedSqlCondition(mode, alias) {
  if (mode === "blocked") return `${alias}.is_blocked = TRUE`;
  if (mode === "unblocked") return `${alias}.is_blocked = FALSE`;
  return "TRUE";
}
...
function publicContributionRank(rank) {
  ...
  return {
    ...
    rankPolicy: rank.rankPolicy,
    contributionComments: rank.contributionComments,
    contributionVisits: rank.contributionVisits,
    contributionParts: rank.contributionParts,
